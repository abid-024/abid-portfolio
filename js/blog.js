  ...
  topicForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (topicMessage) topicMessage.textContent = data.message || "Could not send. Please try again.";
        if (submitButton) submitButton.textContent = "Failed";
      }
    } catch (error) {
      if (topicMessage) topicMessage.textContent = "Could not send. Please try again.";
      if (submitButton) submitButton.textContent = "Failed";
    }
