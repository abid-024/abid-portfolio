(() => {
  ...
  function applyTheme(theme) {
    ...
    document.body.classList.toggle('light', clean === 'light' && document.body.classList.contains('project-vault-page'));

    toggles.forEach((toggle) => {
      toggle.setAttribute('aria-pressed', String(clean === 'dark'));
      toggle.setAttribute('title', clean === 'dark' ? 'Switch to light theme' : 'Switch to dark theme');
    });
    ...
    try {
      ...
      if (storageKey === 'blogTheme') localStorage.setItem('blogDetailTheme', clean);
    } catch (error) {}
  }

  let saved = root.getAttribute('data-theme') || 'light';
  try {
    saved = localStorage.getItem(storageKey) || localStorage.getItem('site-theme') || localStorage.getItem('theme') || saved;
  } catch (error) {}
  applyTheme(saved);
  ...
  function flashProtected(target) {
    const item = target.closest('img, [data-protected-photo], .intro-photo-frame, .portrait-frame, .quote-photo-frame, .hero-photo-card, .post-visual, .project-screen, .screen-frame, .vault-screen, .contact-visual');
    if (!item) return false;
    item.classList.add('is-protected-blur');
    window.clearTimeout(item._protectTimer);
    item._protectTimer = window.setTimeout(() => item.classList.remove('is-protected-blur'), 900);
    return true;
  }
  ...
  function setupCursor() {
    ...
    document.addEventListener('pointerover', (event) => {
      if (event.target.closest('a, button, input, textarea, select, [role="button"]')) cursor.classList.add('is-active');
    });
    document.addEventListener('pointerout', (event) => {
      if (event.target.closest('a, button, input, textarea, select, [role="button"]')) cursor.classList.remove('is-active');
    });
  }
